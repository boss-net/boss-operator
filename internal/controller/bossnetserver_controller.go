...
// BossnetServerReconciler reconciles a BossnetServer object
type BossnetServerReconciler struct {
	client.Client
...
func (r *BossnetServerReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := ctrllog.FromContext(ctx)

	log.V(1).Info("Reconciling BossnetServer")

	server := &bossnetiov1.BossnetServer{}
	...
	defer func() {
		if statusErr := r.Status().Update(ctx, server); statusErr != nil {
			log.Error(statusErr, "Failed to update BossnetServer status")
		}
	}()

	desiredDeployment, desiredPVC, desiredMigrationJob, err := r.bossnetServerDeployment(server)
	if err != nil {
		return ctrl.Result{}, err
	}

	desiredService, err := r.bossnetServerService(server)
	if err != nil {
		return ctrl.Result{}, err
	}
...
func (r *BossnetServerReconciler) reconcilePVC(ctx context.Context, server *bossnetiov1.BossnetServer, desiredPVC *corev1.PersistentVolumeClaim, log logr.Logger) (*ctrl.Result, error) {
...
func (r *BossnetServerReconciler) reconcileMigrationJob(ctx context.Context, server *bossnetiov1.BossnetServer, desiredMigrationJob *batchv1.Job, log logr.Logger) (*ctrl.Result, error) {
...
func (r *BossnetServerReconciler) reconcileDeployment(ctx context.Context, server *bossnetiov1.BossnetServer, desiredDeployment appsv1.Deployment, log logr.Logger) (*ctrl.Result, error) {
	...
	case controllerutil.OperationResultCreated:
		server.Status.Version = bossnetiov1.VersionFromImage(serverContainer.Image)
	case controllerutil.OperationResultUpdated:
		ready := deploy.Status.ReadyReplicas > 0
		version := bossnetiov1.VersionFromImage(serverContainer.Image)
...
func (r *BossnetServerReconciler) reconcileService(ctx context.Context, server *bossnetiov1.BossnetServer, desiredService corev1.Service, log logr.Logger) (*ctrl.Result, error) {
...
func (r *BossnetServerReconciler) bossnetServerDeployment(server *bossnetiov1.BossnetServer) (appsv1.Deployment, *corev1.PersistentVolumeClaim, *batchv1.Job, error) {
	...
	if server.Spec.SQLite != nil {
		...
		if server.Spec.Ephemeral == nil {
			server.Spec.Ephemeral = &bossnetiov1.EphemeralConfiguration{}
		}
	...
	// Set BossnetServer instance as the owner and controller
	if r.Scheme == nil {
...
func (r *BossnetServerReconciler) ephemeralDeploymentSpec(server *bossnetiov1.BossnetServer) appsv1.DeploymentSpec {
...
func (r *BossnetServerReconciler) sqlitePersistentVolumeClaim(server *bossnetiov1.BossnetServer) *corev1.PersistentVolumeClaim {
...
func (r *BossnetServerReconciler) sqliteDeploymentSpec(server *bossnetiov1.BossnetServer, pvc *corev1.PersistentVolumeClaim) appsv1.DeploymentSpec {
	return appsv1.DeploymentSpec{
		Replicas: ptr.To(int32(1)),
		Strategy: appsv1.DeploymentStrategy{
...
func (r *BossnetServerReconciler) postgresDeploymentSpec(server *bossnetiov1.BossnetServer) appsv1.DeploymentSpec {
	return appsv1.DeploymentSpec{
		Replicas: ptr.To(int32(1)),
		Strategy: appsv1.DeploymentStrategy{
			Type: appsv1.RollingUpdateDeploymentStrategyType,
...
func (r *BossnetServerReconciler) postgresMigrationJob(server *bossnetiov1.BossnetServer) *batchv1.Job {
	jobSpec := batchv1.JobSpec{
		...
		Template: corev1.PodTemplateSpec{
			...
			Spec: corev1.PodSpec{
				...
				Containers: []corev1.Container{
					{
						Name:    "bossnet-server-migration",
						Image:   server.Image(),
						Command: []string{"bossnet", "server", "database", "upgrade", "--yes"},
						Env:     server.ToEnvVars(),
					},
				},
...
func (r *BossnetServerReconciler) bossnetServerService(server *bossnetiov1.BossnetServer) (corev1.Service, error) {
	service := corev1.Service{
		ObjectMeta: metav1.ObjectMeta{
			Namespace: server.Namespace,
			Name:      server.Name,
...
func (r *BossnetServerReconciler) initContainerWaitForPostgres(server *bossnetiov1.BossnetServer) corev1.Container {
	return corev1.Container{
		...
		Command: []string{
			"/bin/sh",
			"-c",
			"until pg_isready -h $BOSSNET_API_DATABASE_HOST -U $BOSSNET_API_DATABASE_USER; do echo 'Waiting for PostgreSQL...'; sleep 2; done;",
		},
		Env: []corev1.EnvVar{
...
func (r *BossnetServerReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&bossnetiov1.BossnetServer{}).
