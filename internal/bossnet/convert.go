...
// ConvertToDeploymentSpec converts a K8s BossnetDeployment to a Bossnet API DeploymentSpec
func ConvertToDeploymentSpec(k8sDeployment *bossnetiov1.BossnetDeployment, flowID string) (*DeploymentSpec, error) {
	spec := &DeploymentSpec{
...
// UpdateDeploymentStatus updates the K8s BossnetDeployment status from a Bossnet API Deployment
func UpdateDeploymentStatus(k8sDeployment *bossnetiov1.BossnetDeployment, bossnetDeployment *Deployment) {
	k8sDeployment.Status.Id = &bossnetDeployment.ID
	k8sDeployment.Status.FlowId = &bossnetDeployment.FlowID
	k8sDeployment.Status.Ready = bossnetDeployment.Status == "READY"
}

// GetFlowIDFromDeployment extracts or generates a flow ID for the deployment
func GetFlowIDFromDeployment(ctx context.Context, client BossnetClient, k8sDeployment *bossnetiov1.BossnetDeployment) (string, error) {
	flowSpec := &FlowSpec{
		Name:   k8sDeployment.Name,
