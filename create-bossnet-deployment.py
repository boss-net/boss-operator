...
# ///
"""
Bossnet to Kubernetes CRD Converter

A self-contained PEP 723 script that converts Bossnet flows to BossnetDeployment CRDs,
following the same pattern as `bossnet deploy` but outputting Kubernetes manifests.
...
Basic Usage:
    # Output to stdout
    uv run create-bossnet-deployment.py flows/my_flow.py:my_flow_function \\
        --work-pool my-pool \\
        --server my-bossnet-server \\
        --name my-deployment
...
- No temporary files (unlike `bossnet deploy`)
- Kubernetes native CRD output
- Can be piped directly to `kubectl apply -f -`

Example with all options:
    uv run create-bossnet-deployment.py flows/my_flow.py:my_flow \\
...
import yaml
import typer
from bossnet import flow, Flow
from bossnet.flows import load_flow_from_entrypoint
from bossnet.utilities.callables import parameter_schema_from_entrypoint


app = typer.Typer(
    help="Convert Bossnet flows to Kubernetes BossnetDeployment CRDs",
    epilog="""
Examples:
  ...
  # Full configuration
  uv run create-bossnet-deployment.py flows/my_flow.py:my_flow \\
    --work-pool kubernetes-pool --server my-bossnet-server \\
...
def generate_bossnet_deployment_crd(
    flow_obj: Flow,
    name: str,
    work_pool: str,
    server: str,
    ...
    """Generate a BossnetDeployment CRD manifest"""
...
def main(
    ...
    server: str = typer.Option(
        ...,
        "--server",
        help="Name of the BossnetServer CRD to connect to"
    ),
    ...
    """
    Convert a Bossnet flow to a Kubernetes BossnetDeployment CRD manifest.

    This script introspects a Python flow file and generates a BossnetDeployment
    CRD that can be applied to Kubernetes to deploy the flow using the
    Bossnet Operator.
    """

    try:
        # Load the flow using Bossnet's built-in function
        ...
        # Generate the CRD
        crd = generate_bossnet_deployment_crd(
            flow_obj=flow_obj,
            name=name,
            work_pool=work_pool,
            server=server,
        ...
        if output:
            ...
            typer.echo(f"Generated BossnetDeployment CRD: {output}", err=True)
        else:
            typer.echo(yaml_output)
