...
import (
	...
	"github.com/boss-net/bossnet-operator/internal/controller"
...
func init() {
	...
	utilruntime.Must(bossnetiov1.AddToScheme(scheme))
...
func main() {
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "fd9f6399.bossnet.io",
	...
	if err = (&controller.BossnetServerReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "BossnetServer")
		os.Exit(1)
	}

	if err = (&controller.BossnetWorkPoolReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "BossnetWorkPool")
		os.Exit(1)
	}

	if err = (&controller.BossnetDeploymentReconciler{
		Client:        mgr.GetClient(),
		Scheme:        mgr.GetScheme(),
		BossnetClient: nil, // Will create client dynamically from deployment config
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "BossnetDeployment")
		os.Exit(1)
	}
