...
var _ = Describe("BossnetDeployment type", func() {
	It("can be deep copied", func() {
		original := &BossnetDeployment{
			ObjectMeta: metav1.ObjectMeta{
				Name:      "test-deployment",
				Namespace: "default",
			},
			Spec: BossnetDeploymentSpec{
				...
				WorkPool: BossnetWorkPoolReference{
					Namespace: ptr.To("default"),
					Name:      "kubernetes-work-pool",
					WorkQueue: ptr.To("default"),
				},
				Deployment: BossnetDeploymentConfiguration{
			...
			Status: BossnetDeploymentStatus{
	...
	Context("BossnetServerReference", func() {
		It("should support Bossnet Cloud configuration", func() {
			serverRef := BossnetServerReference{
				RemoteAPIURL: ptr.To("https://api.bossnet.cloud/api/accounts/abc/workspaces/def"),
				AccountID:    ptr.To("abc-123"),
				WorkspaceID:  ptr.To("def-456"),
				APIKey: &APIKeySpec{
			...
			Expect(serverRef.RemoteAPIURL).To(Equal(ptr.To("https://api.bossnet.cloud/api/accounts/abc/workspaces/def")))
		...
		It("should support self-hosted Bossnet server configuration", func() {
			serverRef := BossnetServerReference{
				RemoteAPIURL: ptr.To("https://bossnet.example.com/api"),
				APIKey: &APIKeySpec{
					ValueFrom: &corev1.EnvVarSource{
						SecretKeyRef: &corev1.SecretKeySelector{
							LocalObjectReference: corev1.LocalObjectReference{Name: "bossnet-api-key"},
			...
			Expect(serverRef.RemoteAPIURL).To(Equal(ptr.To("https://bossnet.example.com/api")))
	...
	Context("BossnetWorkPoolReference", func() {
		It("should support namespaced work pool reference", func() {
			workPoolRef := BossnetWorkPoolReference{
				Namespace: ptr.To("bossnet-system"),
				Name:      "kubernetes-work-pool",
				WorkQueue: ptr.To("high-priority"),
			}

			Expect(workPoolRef.Namespace).To(Equal(ptr.To("bossnet-system")))
	...
	Context("BossnetDeploymentConfiguration", func() {
		It("should support basic deployment configuration", func() {
			deploymentConfig := BossnetDeploymentConfiguration{
			...
			Expect(deploymentConfig.Path).To(Equal(ptr.To("/opt/bossnet/flows")))
		...
		It("should support deployment with schedules", func() {
			deploymentConfig := BossnetDeploymentConfiguration{
				Entrypoint: "flows.py:my_flow",
				Schedules: []BossnetSchedule{
					{
						Slug: "daily-schedule",
						Schedule: BossnetScheduleConfig{
		...
		It("should support deployment with concurrency limits", func() {
			deploymentConfig := BossnetDeploymentConfiguration{
				...
				GlobalConcurrencyLimit: &BossnetGlobalConcurrencyLimit{
					Active:             ptr.To(true),
		...
		It("should support deployment with version info", func() {
			deploymentConfig := BossnetDeploymentConfiguration{
				Entrypoint: "flows.py:my_flow",
				VersionInfo: &BossnetVersionInfo{
					Type:    ptr.To("git"),
		...
		It("should support deployment with pull steps", func() {
			pullSteps := []runtime.RawExtension{
				{Raw: []byte(`{"bossnet.deployments.steps.git_clone": {"repository": "https://github.com/example/repo.git"}}`)},
				{Raw: []byte(`{"bossnet.deployments.steps.pip_install_requirements": {"requirements_file": "requirements.txt"}}`)},
			}
	...
	Context("BossnetDeploymentStatus", func() {
				...
				Conditions: []metav1.Condition{
					{
						...
						Message:            "Syncing deployment with Bossnet API",
					},
					{
						Type:               "Synced",
						...
						LastTransitionTime: metav1.Now(),
						Reason:             "SyncComplete",
						Message:            "Successfully synced with Bossnet API",
					},
				},
			}
