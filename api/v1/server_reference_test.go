...
var _ = Describe("BossnetServerReference", func() {
	var (
		ctx       context.Context
	...
	Context("GetAPIURL method", func() {
		It("Should return remote API URL when RemoteAPIURL is set", func() {
			serverRef := &BossnetServerReference{
				RemoteAPIURL: ptr.To("https://api.bossnet.io"),
			}

			apiURL := serverRef.GetAPIURL("test-namespace")
			Expect(apiURL).To(Equal("https://api.bossnet.io/api"))
		})
		...
		It("Should add Bossnet Cloud workspace path when AccountID and WorkspaceID are set", func() {
			serverRef := &BossnetServerReference{
			...
			apiURL := serverRef.GetAPIURL("test-namespace")
			Expect(apiURL).To(Equal("https://api.bossnet.cloud/api/accounts/account-123/workspaces/workspace-456"))
		})
		...
		It("Should build in-cluster service URL when Name is set", func() {
			...
			apiURL := serverRef.GetAPIURL("test-namespace")
			Expect(apiURL).To(Equal("http://bossnet-server.bossnet-system.svc:4200/api"))
		})
		...
		It("Should use fallback namespace when Name is set but Namespace is empty", func() {
			...
			Expect(apiURL).To(Equal("http://bossnet-server.fallback-namespace.svc:4200/api"))
		})
		...
		It("Should prioritize RemoteAPIURL over in-cluster Name", func() {
			serverRef := &BossnetServerReference{
				RemoteAPIURL: ptr.To("https://external.bossnet.io"),
				Name:         "bossnet-server",
				Namespace:    "bossnet-system",
			}

			apiURL := serverRef.GetAPIURL("test-namespace")
			Expect(apiURL).To(Equal("https://external.bossnet.io/api"))
		})
	})
	...
	Context("Helper methods", func() {
		...
		It("Should correctly identify Bossnet Cloud configuration", func() {
			serverRef := &BossnetServerReference{
				RemoteAPIURL: ptr.To("https://api.bossnet.cloud"),
				AccountID:    ptr.To("account-123"),
				WorkspaceID:  ptr.To("workspace-456"),
			...
			Expect(serverRef.IsBossnetCloud()).To(BeTrue())
		})

		It("Should not identify as Bossnet Cloud when AccountID is missing", func() {
			serverRef := &BossnetServerReference{
				RemoteAPIURL: ptr.To("https://api.bossnet.cloud"),
				WorkspaceID:  ptr.To("workspace-456"),
			}

			Expect(serverRef.IsBossnetCloud()).To(BeFalse())
		})

		It("Should not identify as Bossnet Cloud when WorkspaceID is missing", func() {
			serverRef := &BossnetServerReference{
				RemoteAPIURL: ptr.To("https://api.bossnet.cloud"),
				AccountID:    ptr.To("account-123"),
			}
		...
		It("Should not identify as Bossnet Cloud when AccountID is empty", func() {
			serverRef := &BossnetServerReference{
				RemoteAPIURL: ptr.To("https://api.bossnet.cloud"),
				AccountID:    ptr.To(""),
				WorkspaceID:  ptr.To("workspace-456"),
		...
		It("Should not identify as Bossnet Cloud when WorkspaceID is empty", func() {
			serverRef := &BossnetServerReference{
				RemoteAPIURL: ptr.To("https://api.bossnet.cloud"),
				AccountID:    ptr.To("account-123"),
				WorkspaceID:  ptr.To(""),
	...
	Context("Client creation integration", func() {
		It("Should successfully integrate with bossnet client creation", func() {
			// Create a Secret for API key
			secret := &corev1.Secret{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "bossnet-api-secret",
					Namespace: namespace,
				},
				Data: map[string][]byte{
			...
			serverRef := &BossnetServerReference{
				...
				APIKey: &APIKeySpec{
					ValueFrom: &corev1.EnvVarSource{
						SecretKeyRef: &corev1.SecretKeySelector{
							LocalObjectReference: corev1.LocalObjectReference{Name: "bossnet-api-secret"},
							Key:                  "api-key",
						},
					},
				},
			...
			// Verify combination works for Bossnet Cloud
			Expect(serverRef.IsBossnetCloud()).To(BeTrue())
			Expect(serverRef.IsRemote()).To(BeTrue())
			Expect(serverRef.IsInCluster()).To(BeFalse())
		})
