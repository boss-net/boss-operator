...
type BossnetDeploymentSpec struct {
	// Server configuration for connecting to Bossnet API
	Server BossnetServerReference `json:"server"`

	// WorkPool configuration specifying where the deployment should run
	WorkPool BossnetWorkPoolReference `json:"workPool"`

	// Deployment configuration defining the Bossnet deployment
	Deployment BossnetDeploymentConfiguration `json:"deployment"`
}

// BossnetWorkPoolReference defines the work pool for the deployment
type BossnetWorkPoolReference struct {
...
// BossnetDeploymentConfiguration defines the deployment specification
type BossnetDeploymentConfiguration struct {
	...
	// +optional
	VersionInfo *BossnetVersionInfo `json:"versionInfo,omitempty"`
	...
	// +optional
	Schedules []BossnetSchedule `json:"schedules,omitempty"`
	...
	// +optional
	GlobalConcurrencyLimit *BossnetGlobalConcurrencyLimit `json:"globalConcurrencyLimit,omitempty"`
}

// BossnetVersionInfo describes deployment version information
type BossnetVersionInfo struct {
...
type BossnetSchedule struct {
	// Slug is a unique identifier for the schedule
	Slug string `json:"slug"`

	// Schedule defines the schedule configuration
	Schedule BossnetScheduleConfig `json:"schedule"`
}

// BossnetScheduleConfig defines schedule timing configuration
type BossnetScheduleConfig struct {
...
// BossnetGlobalConcurrencyLimit defines global concurrency limit configuration
type BossnetGlobalConcurrencyLimit struct {
...
type BossnetDeploymentStatus struct {
	// Id is the deployment ID from Bossnet
	// +optional
	Id *string `json:"id,omitempty"`

	// FlowId is the flow ID from Bossnet
	// +optional
	...
	// LastSyncTime is the last time the deployment was synced with Bossnet
	// +optional
	...
	// Conditions store the status conditions of the BossnetDeployment instances
	Conditions []metav1.Condition `json:"conditions,omitempty" patchStrategy:"merge" patchMergeKey:"type" protobuf:"bytes,1,rep,name=conditions"`
}

// +kubebuilder:object:root=true
// +kubebuilder:resource:path="bossnetdeployments",singular="bossnetdeployment",shortName="pd",scope="Namespaced"
// +kubebuilder:subresource:status
// +kubebuilder:printcolumn:name="Ready",type="boolean",JSONPath=".status.ready",description="Whether this Bossnet deployment is ready"
// +kubebuilder:printcolumn:name="ID",type="string",JSONPath=".status.id",description="The Bossnet deployment ID"
...
type BossnetDeployment struct {
	metav1.TypeMeta   `json:",inline"`
	metav1.ObjectMeta `json:"metadata,omitempty"`

	Spec   BossnetDeploymentSpec   `json:"spec,omitempty"`
	Status BossnetDeploymentStatus `json:"status,omitempty"`
}

// +kubebuilder:object:root=true
// BossnetDeploymentList contains a list of BossnetDeployment
type BossnetDeploymentList struct {
	metav1.TypeMeta `json:",inline"`
	metav1.ListMeta `json:"metadata,omitempty"`
	Items           []BossnetDeployment `json:"items"`
}

func init() {
	SchemeBuilder.Register(&BossnetDeployment{}, &BossnetDeploymentList{})
}
