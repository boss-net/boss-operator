...
// BossnetServerReference defines how to connect to a Bossnet server
type BossnetServerReference struct {
	// Namespace is the namespace where the in-cluster Bossnet Server is running
	Namespace string `json:"namespace,omitempty"`

	// Name is the name of the in-cluster Bossnet Server in the given namespace
	Name string `json:"name,omitempty"`

	// RemoteAPIURL is the API URL for the remote Bossnet Server. Set if using with an external Bossnet Server or Bossnet Cloud
	...
	// AccountID is the ID of the account to use to connect to Bossnet Cloud
	AccountID *string `json:"accountId,omitempty"`

	// WorkspaceID is the ID of the workspace to use to connect to Bossnet Cloud
	WorkspaceID *string `json:"workspaceId,omitempty"`
}

// APIKeySpec is the API key to use to connect to a remote Bossnet Server
...
func (s *BossnetServerReference) GetAPIURL(fallbackNamespace string) string {
...
// IsRemote returns true if this references a remote Bossnet server
func (s *BossnetServerReference) IsRemote() bool {
	return s.RemoteAPIURL != nil && *s.RemoteAPIURL != ""
}

// IsInCluster returns true if this references an in-cluster Bossnet server
func (s *BossnetServerReference) IsInCluster() bool {
	return s.Name != ""
}

// IsBossnetCloud returns true if this is configured for Bossnet Cloud
func (s *BossnetServerReference) IsBossnetCloud() bool {
...
func (s *BossnetServerReference) GetAPIKey(ctx context.Context, k8sClient client.Client, namespace string) (string, error) {
	if s.APIKey == nil {
		return "", nil
	}
